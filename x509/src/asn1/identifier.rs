...
impl Base128UInt {
    ...
    pub open spec fn spec_serialize(&self, v: UInt, last_byte: bool) -> Seq<u8>
        decreases v via Self::spec_serialize_helper_dereases
    {
        if v == 0 {
            if last_byte {
                seq![0]
    ...
    #[via_fn]
    proof fn spec_serialize_helper_dereases(&self, v: UInt, last_byte: bool)
    {
        assert(v != 0 ==> v >> 7 < v) by (bit_vector);
    }
        ...
        decreases v
    {
        if v == 0 {
            ...
            // if !last_byte {
            //     if let Some(parsed_prefix) = self.spec_parse(prefix, false) {
            //         // // assert(self.spec_parse(s, last_byte).is_some());

            //         // // assert(parsed_prefix == v >> 7);
            //         // assert(v >> 7 <= n_bit_max_unsigned!(8 * uint_size!() - 7)) by (bit_vector);

            //         // assert(s.take(s.len() - 1) == prefix);
            //         // self.lemma_spec_parse_unique_zero_encoding_alt(s);
            //         self.lemma_spec_serialize_non_zero(v);
            //         // assume(take_low_7_bits!(s.first()) != 0);
            //         // assert(self.spec_parse(s, last_byte).is_some());

            //         // let v2 = self.spec_parse(s, last_byte).unwrap();
            //         // assert(v2 == v >> 7 << 7 | take_low_7_bits!(set_high_8_bit!(v)) as UInt);

            //         // assert(
            //         //     v >> 7 << 7 | take_low_7_bits!(take_low_7_bits!(v)) as UInt == v
            //         // ) by (bit_vector);
            ...
            //     // assert(s.len() != 0);
            //     // assert(!is_high_8_bit_set!(s.last()));
            ...
            //         // let v2 = self.spec_parse(s, last_byte).unwrap();
            //         // assert(v2 == v >> 7 << 7 | take_low_7_bits!(take_low_7_bits!(v)) as UInt);
            ...
            // assert(self.spec_parse(self.spec_serialize(v, last_byte), last_byte) == Some(v))
        }
    }
}
