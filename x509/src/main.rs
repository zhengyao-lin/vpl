...
verus! {
    ...
    fn test_x509() -> Result<(), ()> {
        ...
        println_join!("parsed: ", format_dbg(x509::x509_name().parse(issuer.as_slice())?));

        println_join!("parsed: ", format_dbg(x509::x509_algorithm_identifier().parse(
            &[
                0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x0C, 0x05, 0x00
            ]
        )?));

        println_join!("parsed: ", format_dbg(x509::x509_time().parse(
            &[
                0x17, 0x0D, 0x31, 0x36, 0x30, 0x36, 0x32, 0x32, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x5A,
            ]
        )?));

        println_join!("parsed: ", format_dbg(x509::x509_validity().parse(
            &[
        ...
        println_join!("parsed: ", format_dbg(x509::x509_public_key_info().parse(
            &[
        ...
        println_join!("parsed: ", format_dbg(x509::x509_extension().parse(
            &[
