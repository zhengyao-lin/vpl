...
verus! {
    ...
    fn test_x509() -> Result<(), ()> {
        ...
        println_join!("cert: ", format_dbg(ASN1(x509::Certificate).parse(cert.as_slice())?));
        ...
        //     Pair(ASN1(x509::AlgorithmIdentifier), // Signature
        //     Pair(ASN1(x509::Name), // Issuer
        //     Pair(ASN1(x509::Validity), // Validity
        //     Pair(ASN1(x509::Name), // Subject
        //     Pair(ASN1(x509::PublicKeyInfo), // Subject Public Key Info

        //     Optional::new(ASN1(ImplicitTag(TagValue {
        //         class: TagClass::ContextSpecific,
        ...
        //     Optional::new(ASN1(ExplicitTag(TagValue {
        //         class: TagClass::ContextSpecific,
        //         form: TagForm::Constructed,
        //         num: 3,
        //     }, ASN1(x509::Extensions))), // Extensions
        ...
        // println_join!("len: ", len, ", parsed: ", format_dbg(res));
        ...
        // println_join!("parsed name: ", format_dbg(ASN1(x509::Name).parse(issuer.as_slice())?));

        // println_join!("parsed algo id: ", format_dbg(Pair(ASN1(x509::AlgorithmIdentifier), End).parse(
        //     &[
        //         0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x0C, 0x05, 0x00
        //     ]
        // )?));

        // println_join!("parsed time: ", format_dbg(x509::Time.parse(
        //     &[
        //         0x17, 0x0D, 0x31, 0x36, 0x30, 0x36, 0x32, 0x32, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x5A,
        //     ]
        // )?));

        // println_join!("parsed validity: ", format_dbg(ASN1(x509::Validity).parse(
        //     &[
        ...
        // println_join!("parsed pub key info: ", format_dbg(ASN1(x509::PublicKeyInfo).parse(
        //     &[
        ...
        // println_join!("parsed ext: ", format_dbg(ASN1(x509::Extension).parse(
        //     &[
        ...
        //     println_join!("disjoint: ", Optional::new(ASN1(ExplicitTag(TagValue {
        //         class: TagClass::ContextSpecific,
        //         form: TagForm::Constructed,
        //         num: 3,
        //     }, ASN1(x509::Extensions))), // Extensions

        // End).disjoint_from(&ASN1(ImplicitTag(TagValue {
        //     class: TagClass::ContextSpecific,
        //     form: TagForm::Primitive,
        //     num: 2,
        // }, BitString))));

        //     println_join!("parsed ext: ", format_dbg(Optional::new(ASN1(ImplicitTag(TagValue {
        //             class: TagClass::ContextSpecific,
        //             form: TagForm::Primitive,
        //             num: 2,
        //         }, BitString)), // Subject UID
        ...
        // println_join!("tbs cert: ", format_dbg(ASN1(x509::TBSCertificate).parse(
        //     &[
        //         ,
        //     ]
        // )?));
